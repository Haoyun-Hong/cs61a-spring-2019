...
def find_closest(location, centroids):
    ...
    # BEGIN Question 3
    "*** YOUR CODE HERE ***"
    record_min=centroids[0]
    for centroid in centroids:
        if distance(location, centroid) < distance(location, record_min):
            record_min=centroid
    return record_min
    # END Question 3
...
def group_by_centroid(restaurants, centroids):
    ...
    # BEGIN Question 4
    "*** YOUR CODE HERE ***"
    #Be careful, list+list is different from list.append 
    result=[]
...
def find_centroid(cluster):
    ...
    for restaurant in cluster:
        location_lon.append(restaurant_location(restaurant)[0])
        location_lat.append(restaurant_location(restaurant)[1])
    return [mean(location_lon), mean(location_lat)]
    # END Question 5
...
def find_predictor(user, restaurants, feature_fn):
    ...
    for i, k in zip(xs, ys):
        S_xy += (i-mean(xs))*(k-mean(ys))
...
def best_predictor(user, restaurants, feature_fns):
    ...
    for feature_fn in feature_fns: 
        result [find_predictor(user, reviewed, feature_fn)[1]]= find_predictor(user, reviewed, feature_fn)[0]
...
def rate_all(user, restaurants, feature_fns):
    ...
    for restaurant in restaurants:
        if restaurant in reviewed:
            ratings_final[restaurant_name(restaurant)]=user_rating(user, restaurant_name(restaurant))
        else:
            ratings_final[restaurant_name(restaurant)]=predictor(restaurant)
