...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self,hive):
        ...
        the ThrowerAnt's Place by following entrance. 
        This method returns None if there is no such Bee (or none in range)."""
        # BEGIN Problem 3 and 4
        i=0
        place=self.place 
        check_bees=random_or_none(place.bees)
        while True: 
            if i >= self.min_range and i <= self.max_range:
                if check_bees is None and place.entrance is not hive:
                    i+=1
                    place=place.entrance 
                    check_bees=random_or_none(place.bees)
                else:
                    return check_bees
            elif i < self.min_range:
                i+=1
                place=place.entrance 
                check_bees=random_or_none(place.bees)
            else:
...
class BodyguardAnt(Ant):
    ...
    def can_contain(self, other):
        # BEGIN Problem 9
        "*** YOUR CODE HERE ***"
        if other.is_container==False or other.contained_ant is not None:
            return True 
        else:
            return False
        # END Problem 9
        ...
        if self.is_imposter:
            ...
            while place is not None:
                if place.ant is not None: 
                    if place.ant.is_container == True and place.ant.contained_ant is not None:
                        if place.ant.contained_ant not in self.ant:
                            place.ant.contained_ant.damage= place.ant.contained_ant.damage *2
                            self.ant.append(place.ant.contained_ant)
                    if place.ant not in self.ant:
                        place.ant.damage = place.ant.damage *2
                        self.ant.append (place.ant)
                place=place.exit 
        # END Problem 13
