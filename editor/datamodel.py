...
class Pair(Expression):
    ...
    def __repr__(self):
        if isinstance(self.rest, Pair):
            return ''.join(['(', '{}'.format(repr(self.first)), ' ', '{}'.format(repr(self.rest)[1:(- 1)]), ')'])
        elif (self.rest is Nil):
            return ''.join(['(', '{}'.format(repr(self.first)), ')'])
        else:
            return ''.join(['(', '{}'.format(repr(self.first)), ' . ', '{}'.format(repr(self.rest)), ')'])
